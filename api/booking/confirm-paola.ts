...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!id || typeof id !== "string") {
    return res.status(400).send(page("Link inválido", "El enlace de confirmación no es válido.", false));
  }
  ...
  if (reserva.status === "confirmada") {
    return res.status(200).send(page("Ya confirmada", `Tu reserva de <strong>${reserva.service_name}</strong> ya estaba confirmada. ¡Te esperamos!`, true));
  }

  if (reserva.status === "cancelada") {
    return res.status(200).send(page("Reserva cancelada", "Esta reserva fue cancelada y ya no está disponible.", false));
  }
  ...
  const dateStr = reserva.date
    ? new Date(reserva.date).toLocaleString("es-CL", {
  ...
  return res.status(200).send(
    page("¡Reserva confirmada!", `Tu <strong>${reserva.service_name}</strong> el <strong>${dateStr}</strong> está confirmada. ¡Te esperamos!`, true),
  );
}

function page(title: string, message: string, success: boolean) {
  const color = success ? "#2E3135" : "#7f1d1d";
  const icon  = success ? "✓" : "✕";
  return `<!DOCTYPE html>
<html lang="es">
<head>
  ...
  <title>${title} — Dra. Paola Cárdenas Fica</title>
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    body{font-family:'Helvetica Neue',Arial,sans-serif;background:#F8F6F2;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px}
    ...
    p{font-size:15px;color:#5A5148;line-height:1.6}
...
<body>
  <div class="card">
    ...
    <h1>${title}</h1>
    <p>${message}</p>
    <a href="https://drapaolacardenasfica.com">Volver al inicio</a>
  </div>
</body>
</html>`;
}
