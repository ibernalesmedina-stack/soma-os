...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Also update pago linked to this reserva
  await fetch(`${SUPABASE_URL}/rest/v1/pagos?reserva_id=eq.${id}`, {
    method: "PATCH",
    headers: {
      apikey: SERVICE_KEY, Authorization: `Bearer ${SERVICE_KEY}`,
      "Content-Type": "application/json", Prefer: "return=minimal",
  ...
  return res.status(200).send(
    page("¡Reserva confirmada!", `Tu hora de <strong>${reserva.service_name}</strong> el <strong>${dateStr}</strong> está confirmada. El pago se realiza el día de la consulta. ¡Te esperamos!`, true),
  );
}
